...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f7faff] dark:bg-slate-950 text-slate-900 dark:text-slate-50">
    ...
    <aside className={cn('fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-white dark:bg-slate-900 px-5 py-6 shadow-xl transition-transform lg:hidden', open ? 'translate-x-0' : '-translate-x-full')}>
      <div className="mb-8 flex items-center justify-between"><Link href="/dashboard" onClick={close} className="flex items-center gap-2.5"><span className="flex size-10 items-center justify-center rounded-2xl bg-blue-600 text-white"><Heart className="size-5 fill-current" /></span><span className="text-xl font-bold">kudos</span></Link><button aria-label="Close menu" onClick={close} className="rounded-lg p-2 text-slate-400 dark:text-slate-500"><X className="size-5" /></button></div><Nav pathname={pathname} onNavigate={close} badges={badges} />
    </aside>
