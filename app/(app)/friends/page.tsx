...
export default function FriendsPage() {
  ...
  function relationshipAction(person: UserProfile) {
    ...
    return <button onClick={withPending(person.uid, () => sendFriendRequest(user.uid, person.uid))} disabled={pending.has(person.uid)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 disabled:opacity-50">{pending.has(person.uid) ? 'Sending…' : 'Add friend'}</button>
  }
  ...
  return <div>
    ...
    {searchResult && <div className="mb-6 max-w-xl rounded-2xl border border-blue-100 bg-blue-50/50 p-4">
      ...
      {actionError[searchResult.uid] && <p className="mt-2 text-right text-xs text-red-600">{actionError[searchResult.uid]}</p>}
    </div>}
    ...
    {tab === 'Requests' && <PeopleGrid empty="No pending requests." people={incoming.map(r => ({
      ...
      action: <div className="flex gap-2">
        ...
        <button onClick={withPending(r.id, () => declineFriendRequest(r.id))} disabled={pending.has(r.id)} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600 disabled:opacity-50">{pending.has(r.id) ? '…' : 'Decline'}</button>
      </div>,
    }))} />}
    ...
    {tab === 'Sent' && <PeopleGrid empty="No pending sent requests." people={sent.map(r => ({
      ...
      name: r.profile?.name || 'Unknown',
      error: actionError[r.id],
      action: <button onClick={withPending(r.id, () => declineFriendRequest(r.id))} disabled={pending.has(r.id)} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600 disabled:opacity-50">{pending.has(r.id) ? '…' : 'Cancel'}</button>,
    }))} />}
    ...
    {tab === 'Suggestions' && <PeopleGrid empty="No suggestions right now." people={suggestions.map(p => ({
      ...
      action: <button onClick={withPending(p.uid, () => user ? sendFriendRequest(user.uid, p.uid) : Promise.resolve())} disabled={pending.has(p.uid)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 disabled:opacity-50">{pending.has(p.uid) ? 'Sending…' : 'Add friend'}</button>,
    }))} />}
  </div>
}
...
function PeopleGrid({ people, empty }: { people: { uid: string; name: string; action: React.ReactNode; error?: string }[]; empty: string }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-2">
    {people.map(p => <div key={p.uid} className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex items-center gap-3">
